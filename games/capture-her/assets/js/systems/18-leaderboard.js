...
async function showLeaderboard() {
  ...
  const rankEmojis = ['🥇','🥈','🥉'], rankClasses = ['r1','r2','r3'];
  ...
  let comments = {};
  if(localStorage.getItem('LW_API_KEY') && ranking.length) {
    try {
...
返回JSON：{"${ranking[0]?.charId||''}":"评语","${ranking[1]?.charId||''}":"评语","${ranking[2]?.charId||''}":"评语","failed":"对未完成者的总结评语"}`;
  ...
  if(!ranking.length) {
    ...
    html = ranking.slice(0,3).map((r,i)=>{
      ...
      return `<div class="lb-row"><div class="lb-rank ${rankClasses[i]||''}">${rankEmojis[i]||'·'}</div><div style="flex:1"><div class="lb-id">${c?.chatId||r.charId}</div><div class="lb-comment">${comment}</div></div><div style="font-family:var(--mono);font-size:9px;color:var(--text3)">${timeStr}</div></div>`;
    }).join('');
  }
  if(failedChars.length) html += `<div style="padding:10px 16px;font-family:var(--mono);font-size:9px;color:var(--text3);border-top:1px solid var(--border)">未完成：${failedChars.join(' · ')} · ${comments.failed||'系统已记录'}</div>`;
  lb.innerHTML = html;
