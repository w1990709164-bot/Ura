...
function processHorae(h, currentTask){
  ...
  if(h.char_id && G.chars[h.char_id]){
    ...
    if(h.char_os) {
      cs.os = h.char_os;
      if(currentTask && currentTask.charId === h.char_id && typeof addCharPov === 'function') {
        addCharPov(h.char_id, h.char_os, 'thought');
      }
    }
  ...
  if(h.task_complete && currentTask && !currentTask.done){
    if((currentTask.roundCount||0) < 2) {
      ...
      showToast(`任务完成：${c?.name || currentTask.charId} · 好感 +2`);
  ...
  if(h.social_death && h.char_id){
    const forced = h.social_death_action || '系统强制触发了一次社死行动';
