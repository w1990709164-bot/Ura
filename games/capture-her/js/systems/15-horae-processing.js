...
function processHorae(h, currentTask){
  ...
  // Increment round count for current task scene
  if(currentTask) {
    currentTask.roundCount = (currentTask.roundCount||0) + 1;
  }

  if(h.task_complete && currentTask && !currentTask.done){
    // Enforce minimum 2 rounds before task can complete
    if((currentTask.roundCount||0) < 2) {
      ...
      // Give gold reward for completed task
      const goldReward = 30 + Math.floor(Math.random()*50);
      G.wallet += goldReward;
      updateTopBar();
      // Show goodfeel picker after a longer delay so player can read the scene
