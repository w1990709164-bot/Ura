...
function loadGlobalApiIntoState(){
  API.url   = localStorage.getItem('LW_API_URL')   || '';
  API.key   = localStorage.getItem('LW_API_KEY')   || '';
  API.model = localStorage.getItem('LW_API_MODEL') || 'gpt-4o';
}
function persistGlobalApi(){
  localStorage.setItem('LW_API_URL',   API.url   || '');
  localStorage.setItem('LW_API_KEY',   API.key   || '');
  localStorage.setItem('LW_API_MODEL', API.model || '');
}

async function callAI(messages, systemPrompt, maxTokens=600){
  const cfg = getGlobalApi();
  if(!cfg.key) throw new Error('未设置 API Key，请返回主页设置 API');
  const isOpenAI = !!cfg.url;
  const model = cfg.model || (isOpenAI ? 'gpt-4o' : 'claude-sonnet-4-20250514');
