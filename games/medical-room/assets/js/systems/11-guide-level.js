...
const LEVEL_REQUIREMENTS = {
  '见习':  {nextLevel:'初级', sessions:5,  phase:2, phaseCount:1, salaryBase:200},
  '初级':  {nextLevel:'中级', sessions:15, phase:4, phaseCount:2, salaryBase:300},
  '中级':  {nextLevel:'高级', sessions:30, phase:7, phaseCount:1, salaryBase:450},
  '高级':  {nextLevel:'首席', sessions:50, phase:9, phaseCount:1, salaryBase:600},
  '首席':  {nextLevel:null, sessions:999, phase:10, phaseCount:1, salaryBase:800},
};
...
function checkLevelUp() {
  ...
  // Prevent double-trigger: if we already recorded this level, skip
  ...
  if (totalSessions >= req.sessions && highPhaseCount >= req.phaseCount) {
    G.playerStats.lastLeveledAt = level; // mark current level as "upgrade triggered"
    G.playerStats.level = req.nextLevel;
    saveGame();
    showLevelUpNotice(level, req.nextLevel, req.salaryBase * 5);
  }
}

function showLevelUpNotice(oldLevel, newLevel, bonusAmount) {
  const bonus = bonusAmount || 1000;
  ...
  modal.innerHTML = `
    ...
    <div style="font-family:'Share Tech Mono',monospace;font-size:13px;color:#00ffcc;margin-bottom:16px;padding:8px;background:rgba(0,255,204,0.06);border:1px solid rgba(0,255,204,0.2);border-radius:4px">
      + ${bonus.toLocaleString()} 积分 · 晋升奖励
    </div>
    <button style="width:100%;padding:10px;background:rgba(0,229,255,0.1);border:1px solid #00e5ff;border-radius:4px;color:#00e5ff;font-family:'Rajdhani',sans-serif;font-size:13px;font-weight:700;cursor:pointer;letter-spacing:2px">
      确认领取
    </button>
  `;
  modal.querySelector('button').onclick = () => { bg.remove(); modal.remove(); };
  document.body.appendChild(modal);
  bg.onclick = () => { bg.remove(); modal.remove(); };
  // Give real bonus
  addSalary(bonus, `晋升为${newLevel}向导医生`);
  showToast(`⭐ 职级晋升：${newLevel} · +${bonus}积分`);
}
...
function showBagUnlock(charId, bagNum, c) {
  ...
  modal.innerHTML = `
    ...
    <button onclick="document.getElementById('bag-unlock-modal').remove();openArchiveForChar('${charId}')"
      style="width:100%;padding:10px;background:var(--cyan-glow,rgba(0,229,255,0.12));border:1px solid var(--cyan,#00e5ff);border-radius:4px;color:var(--cyan,#00e5ff);font-family:'Rajdhani',sans-serif;font-size:13px;font-weight:700;cursor:pointer;letter-spacing:2px;margin-bottom:8px">
      查看档案袋
    </button>
    <button onclick="document.getElementById('bag-unlock-modal').remove()"
      style="width:100%;padding:8px;background:none;border:1px solid var(--border,#1a2840);border-radius:4px;color:var(--text3,#2a4050);font-family:'Share Tech Mono',monospace;font-size:11px;cursor:pointer">
      稍后查看
    </button>
  `;
  document.body.appendChild(modal);
  ...
  modal.querySelector('button').onclick = () => {
    ...
    openArchiveForChar(charId);
  };
  modal.querySelectorAll('button')[1].onclick = () => { bg.remove(); modal.remove(); };
}

function openArchiveForChar(charId) {
  // Open archive overlay and go directly to character
  openOverlay('archive-panel');
  renderArchive();
  setTimeout(() => openCharArchive(charId), 100);
}

// Spirit state system - per character based on their actual status
const SPIRIT_BEHAVIORS = {
  // [location/status] -> behavior description
...
function getSpiritBehaviorPrompt(charId) {
  ...
  // Determine behavior
  let behavior = '';
  if (loc.includes('战斗') || loc.includes('任务')) {
    behavior = SPIRIT_BEHAVIORS.combat(spiritName);
  } else if (loc.includes('睡眠') || loc.includes('睡觉')) {
    behavior = SPIRIT_BEHAVIORS.sleep(spiritName);
  } else if (loc.includes('休息')) {
    behavior = SPIRIT_BEHAVIORS.rest(spiritName);
  } else if (loc.includes('巡逻') || loc.includes('待命')) {
    behavior = SPIRIT_BEHAVIORS.patrol(spiritName);
  } else if (G.currentLocation === 'clinic' && G.clinicSession === charId) {
    if (trust < 30) behavior = SPIRIT_BEHAVIORS.clinic_low(spiritName);
    else if (trust < 65) behavior = SPIRIT_BEHAVIORS.clinic_mid(spiritName);
    else behavior = SPIRIT_BEHAVIORS.clinic_high(spiritName);
  } else if (trust > 60 && Math.random() < 0.3) {
    behavior = SPIRIT_BEHAVIORS.trust_visit(spiritName);
  } else {
    behavior = SPIRIT_BEHAVIORS.patrol(spiritName);
  }
  return behavior;
...
function triggerEmergency(charId, c, tmpl) {
  // Update schedule with urgent entry — respect daily cap
  if (!G.schedule.find(s => s.id === charId && s.type === 'urgent')) {
    if (countUrgentToday() < MAX_URGENT_PER_DAY) {
      G.schedule.unshift({
        id: charId,
        type: 'urgent',
        time: '紧急',
  ...
  if (el) {
    el.textContent = `⚡ ${c.name} · ${tmpl.title}`;
    el.style.color = 'var(--pink, #ff2d6b)';
  }

  // Add narration in scene
  addSysMsg('⚡ 紧急通报', tmpl.generateMsg(c));

  // Add to AI history for context
  G.history.push({
    role: 'user',
    content: `[紧急事件] ${tmpl.generateMsg(c)} 请生成紧急疏导场景，向导需要立即介入。`
  });
