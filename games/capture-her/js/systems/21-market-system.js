...
function openMarket() {
  ...
  modal.innerHTML = `
    <div style="height:52px;background:var(--panel);border-bottom:1px solid var(--border2);display:flex;align-items:center;padding:0 16px;flex-shrink:0;gap:12px">
      <div style="font-family:var(--display);font-size:16px;font-weight:700;color:var(--white);flex:1">🛍 补给站</div>
      <div style="font-family:var(--mono);font-size:12px;color:var(--gold)">💳 ${G.wallet}</div>
      <div onclick="document.getElementById('market-modal').remove()" style="width:32px;height:32px;background:var(--bg3);border:1px solid var(--border);border-radius:2px;color:var(--text2);font-size:14px;cursor:pointer;display:flex;align-items:center;justify-content:center">✕</div>
    </div>
    <div style="font-family:var(--mono);font-size:9px;color:var(--text3);letter-spacing:2px;padding:8px 16px;border-bottom:1px solid var(--border)">本周货品 · 每天刷新</div>
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:8px" id="market-items-list">
      ${weeklyMarketItems.map(item=>`
        <div style="background:var(--panel);border:1px solid var(--border);border-radius:3px;padding:12px 14px;display:flex;align-items:center;gap:12px">
          <div style="font-size:22px;flex-shrink:0">${item.icon}</div>
          <div style="flex:1;min-width:0">
            <div style="font-family:var(--mono);font-size:12px;font-weight:600;color:var(--white);margin-bottom:2px">${item.name}</div>
            <div style="font-family:var(--mono);font-size:9px;color:var(--text2)">${item.desc}</div>
            ${item.bondBonus?`<div style="font-family:var(--mono);font-size:9px;color:var(--red);margin-top:2px">送人好感 +${item.bondBonus}</div>`:''}
          </div>
          <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;flex-shrink:0">
            <div style="font-family:var(--mono);font-size:12px;font-weight:700;color:var(--gold)">💳 ${item.price}</div>
            <div onclick="buyItem('${item.id}')" style="font-family:var(--mono);font-size:9px;letter-spacing:1px;color:${G.wallet>=item.price?'var(--red)':'var(--text3)'};border:1px solid ${G.wallet>=item.price?'var(--red-dim)':'var(--border)'};border-radius:2px;padding:3px 10px;cursor:${G.wallet>=item.price?'pointer':'default'};background:${G.wallet>=item.price?'var(--red-glow2)':'none'}">购买</div>
          </div>
        </div>`).join('')}
    </div>`;
...
function buyItem(itemId) {
  ...
  const purchased = {
    ...
    bondBonus: item.bondBonus, bonus: item.bonus,
    statBoost: item.statBoost, statVal: item.statVal
  };
