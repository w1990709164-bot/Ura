...
function generateDailyTasks(cb) {
  // 2-4 chars get tasks today, randomly selected
  ...
  G.todayRanking = [];
  // Generate task descriptions via AI
...
[{"charId":"角色id","task":"任务描述，要具体有趣，不超过30字"}]`;
  ...
  // Show morning notification and refresh pov headers with real task descs
  showMorningNotifs();
  refreshCharPovTaskHeaders(); // immediate refresh after AI writes taskDesc
  saveGame();
  if(cb) cb();
}

function showMorningNotifs() {
  // Initialize charPovData immediately for all task chars
  G.dailyTasks.forEach((t,i)=>{
    if(!charPovData[t.charId]) charPovData[t.charId] = [];
    // Set first task char as default
    if(i===0) currentCharPovId = t.charId;
  });

  // After task descs are generated, update charPovData
  // This is called after generateTaskDescs resolves
  refreshCharPovTaskHeaders();

  // Show brief scene notification
  setTimeout(()=>{
...
function refreshCharPovTaskHeaders() {
  // Update task descriptions in charPovData after AI generates them
  G.dailyTasks.forEach(t=>{
    if(!charPovData[t.charId]) charPovData[t.charId] = [];
    // Remove old task entries and re-add with latest desc
  ...
  // Refresh display if currently viewing a task char's pov
