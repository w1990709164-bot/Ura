...
function renderApPanel() {
  ...
  if (G.apPending) {
    if (pending) {
      pending.innerHTML =
        `<div style="margin-bottom:6px;">⏳ 正在进行：${G.apPending.label} — 请继续对话推进剧情，出现达成提示后将自动完成</div>` +
        `<div style="display:flex;gap:8px;justify-content:center;">` +
        `<button onclick="forceCompleteAp()" class="ap-free-btn" style="font-size:0.6rem;padding:4px 10px;">结束本月行动</button>` +
        `<button onclick="cancelAp()" class="ap-free-btn" style="font-size:0.6rem;padding:4px 10px;opacity:0.65;">取消</button>` +
        `</div>`;
...
// 检测AI叙事中的达成标记，消耗行动点
// ══════════════════════════════
const AP_AUTO_COMPLETE_TURNS = 4; // 软锁兜底：AI 连续 N 轮仍未给达成标记则自动完成本月
...
function checkApCompletion(narrativeText, horae) {
  ...
  // 未达成且未到兜底轮次：继续对话推进剧情
  if (!hasAchieve && p.turns < AP_AUTO_COMPLETE_TURNS) return;

  _applyApReward(p, horae);
}

// 实际发放行动点奖励并消耗点数。horae 若存在，会被清除 bond_delta/stat_gain，
// 避免随后 processHorae 再次叠加（行动点固定增长为准，杜绝翻倍）。
function _applyApReward(p, horae) {
  if (!G.apUsedThisYear) G.apUsedThisYear = [];
...
// 软锁兜底：手动结束本月行动（玩家点击横幅按钮时调用）
function forceCompleteAp() {
  if (!G.apPending) return;
  _applyApReward(G.apPending, null);
}

// 取消当前待完成行动点（不消耗点数，回到选择面板）
function cancelAp() {
  if (!G.apPending) return;
  G.apPending = null;
  saveGame();
