  ...
  list.innerHTML=items.map((item,idx)=>`
    <div class="bp-item">
      ...
      <div class="bp-info">
        ...
        <div class="bp-desc">${item.desc||''}</div>
        ${item.sender?`<div class="bp-sender">来自：${item.sender}</div>`:''}
      </div>
      <div class="bp-use" onclick="useBackpackItem(${idx})">使用</div>
    </div>`).join('');
}
...
function useBackpackItem(idx){
  ...
  if(item.type==='dice') {
    G.activeDiceBonus = (G.activeDiceBonus||0)+(item.bonus||3);
    showToast('✓ '+item.name+' · 下次检定 +'+(item.bonus||3));
