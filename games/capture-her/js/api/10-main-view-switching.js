...
function showSecondTab() {
  // Update nav label based on phase
...
let charPovData = {}; // charId -> [{type, content}]
...
function renderCharPovContent(charId) {
  ...
  // Render thought bubbles and narration
  data.forEach(msg => {
    if(msg.type === 'task') return; // skip, already shown in header
...
function addCharPov(charId, content, type='thought') {
  ...
  // If currently viewing this char's POV, update display
  if(currentCharPovId === charId) renderCharPovContent(charId);
  // Also add badge indicator on nav
  const icon = document.getElementById('nav-second-icon');
...
function enableDualView(on) {
  G.isDualView = on;
  // No longer splits the screen - user POV stays in scene, char POV goes to second tab
