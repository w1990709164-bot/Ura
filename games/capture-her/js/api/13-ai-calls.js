...
async function callAI(){
  ...
  document.getElementById('send-btn').disabled=true;
  const thinkId=addThinking();
  // Capture currentTask at call time so closure is correct
...
async function fetchAI(systemPrompt, history){
  ...
  const model=G.apiModel||'claude-sonnet-4-20250514';
  let response, data;
  if(isOpenAI){
    response=await fetch(endpoint,{
      method:'POST',
      headers:{'Content-Type':'application/json','Authorization':`Bearer ${G.apiKey}`},
      body:JSON.stringify({model,max_tokens:2500,messages:[{role:'system',content:systemPrompt},...history]})
    });
    ...
    response=await fetch(endpoint,{
      ...
      body:JSON.stringify({model,max_tokens:2500,system:systemPrompt,messages:history})
    });
    data=await response.json();
